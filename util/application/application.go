...
type Application struct {
	...
	log               logger.LeveledLogger  // Default application logger
...
type Options struct {
	...
	CustomLogger logger.LeveledLogger
...
func Create(ops Options) (*Application, error) {
	...
	*app.targetFPS = 60
	...
	app.log.Infof("OpenGL version: %s", glVersion)
...
func (app *Application) Log() logger.LeveledLogger {
...
func (app *Application) Run() error {
	...
	if *app.swapInterval >= 0 {
		...
		app.log.Debugf("Swap interval set to: %v", *app.swapInterval)
	}
	...
	if *app.cpuProfile != "" {
		...
		app.log.Infof("Started writing CPU profile to: %s", *app.cpuProfile)
	}
	...
	if *app.execTrace != "" {
		...
		defer trace.Stop()
		app.log.Infof("Started writing execution trace to: %s", *app.execTrace)
	}
...
func (app *Application) OpenDefaultAudioDevice() error {
	...
	app.log.Infof("%s version: %s", al.GetString(al.Vendor), al.GetString(al.Version))
	app.log.Infof("%s", vorbis.VersionString())
	return nil
}
